...
class TodoApi extends ProductBase<ITodo> {
	...
	searchTask = (searchText: string, callback: (error: Meteor.Error, result: ITodo[]) => void) => {
		this.callMethod('searchTask', searchText, callback);
	};
	getUsernameById = (userId: string, callback: (error: Meteor.Error, result: string) => void) => {
		this.callMethod('getUsernameById', userId, callback);
	};
